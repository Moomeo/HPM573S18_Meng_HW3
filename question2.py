...
class Node:
    ...
    def get_expected_utility(self):
        ...
        :return: health utility of this node
        """
        raise NotImplementedError("This is an abstract method and needs to be implemented in derived classes.")
...
class ChanceNode(Node):
    ...
    def get_expected_utility(self):
        exp_uti = self.utility
        i = 0
        for node in self.futureNodes:
            exp_uti +=self.probs[i]*node.get_expected_utility()
            i += 1
        return exp_uti
...
class DecisionNode(Node):
    ...
    def get_expected_utility(self):
        out = dict()
        for node in self.futureNode:
            out[node.name] = node.get_expected_utility
...
print(C2.get_expected_cost())
print(C2.get_expected_utility())